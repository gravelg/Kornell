...
function doLMSInitialize()
{
   ...
   if (api == null)
   {
      !window.isPreview && console.warn("Unable to locate the LMS's API Implementation.\nLMSInitialize was not successful.");
      return "false";
   }
...
function doLMSGetValue(name)
{
   ...
   else
   {
      ...
      if (errCode != _NoError)
      {
         ...
         !window.isPreview && console.warn("LMSGetValue("+name+") failed. \n"+ errDescription);
         return "";
      }
      else
      {
...
function ErrorHandler()
{
   ...
   if (errCode != _NoError)
   {
      ...
      !window.isPreview && console.warn(errDescription);
   }
...
function findAPI(win)
{
   while ((win.API == null) && (win.parent != null) && (win.parent != win))
   {
      ...
      if (findAPITries > 7) 
      {
         !window.isPreview && console.warn("Error finding API -- too deeply nested.");
         return null;
      }
...
function getAPI()
{
   ...
   if (theAPI == null)
   {
      !window.isPreview && console.warn("Unable to find an API adapter");
   }
   return theAPI
}
