...
public class CourseClassClient extends RESTClient {
    ...
    public void update(CourseClass courseClass, Callback<CourseClass> cb) {
        PUT("courseClasses", courseClass.getUUID()).withContentType(CourseClass.TYPE).withEntityBody(courseClass)
                .go(cb);
    }
    ...
    public void getObservers(String bindMode, Callback<RolesTO> cb) {
        GET("courseClasses", courseClassUUID, "observers" + "?bind=" + bindMode).withContentType(CourseClass.TYPE)
                .go(cb);
    }
