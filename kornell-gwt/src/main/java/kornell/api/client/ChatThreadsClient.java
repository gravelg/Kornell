...
public class ChatThreadsClient extends RESTClient {
    ...
    public void postMessageToSupportCourseClassThread(String message, String courseClassUUID,
            Callback<String> callback) {
        POST("/chatThreads/courseClass/" + courseClassUUID + "/support").sendRequest(message, callback);
    }

    public void postMessageToTutoringCourseClassThread(String message, String courseClassUUID,
            Callback<String> callback) {
    ...
    public void postMessageToChatThread(String message, String chatThreadUUID,
            Callback<ChatThreadMessagesTO> callback) {
        postMessageToChatThread(message, chatThreadUUID, null, callback);
    }

    public void postMessageToChatThread(String message, String chatThreadUUID, Date since,
            Callback<ChatThreadMessagesTO> callback) {
        POST("/chatThreads/" + chatThreadUUID + "/message/?since=" + ((since != null) ? since.getTime() : ""))
    ...
    public void getChatThreadMessages(String chatThreadUUID, Date since, Date before,
            Callback<ChatThreadMessagesTO> callback) {
        GET("/chatThreads/" + chatThreadUUID + "/messages/?since=" + ((since != null) ? since.getTime() : "")
                + "&before=" + ((before != null) ? before.getTime() : "")).sendRequest(null, callback);
    }
