...
public class GenericPasswordChangeView extends Composite implements ProfileView {
    ...
    public void initData(KornellSession session, EventBus bus, UserInfoTO user) {
        ...
        modalNewPassword = new KornellFormFieldWrapper(constants.newPassword(),
                formHelper.createPasswordTextBoxFormField(""), true);
        fields.add(modalNewPassword);
        passwordChangeFields.add(modalNewPassword);

        modalNewPasswordConfirm = new KornellFormFieldWrapper(constants.confirmPassword(),
                formHelper.createPasswordTextBoxFormField(""), true);
    ...
    void doOK(ClickEvent e) {
        ...
        if (validateFields()) {
            ...
            session.user().changeTargetPassword(user.getPerson().getUUID(), modalNewPassword.getFieldPersistText(),
                    new Callback<Void>() {
                        @Override
