...
public class GenericCourseSummaryView extends Composite {
	...
	private void onEnrolledOrNot() {
		imgThumb.setUrl(StringUtils.mkurl("/",courseClassTO.getCourseVersionTO().getDistributionURL(),courseClassTO.getCourseVersionTO().getCourse().getCode(),
				courseClassTO.getCourseVersionTO().getCourseVersion().getDistributionPrefix(),"/classroom/images/thumb.jpg"));
