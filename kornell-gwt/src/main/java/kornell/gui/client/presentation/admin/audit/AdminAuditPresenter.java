...
public class AdminAuditPresenter extends PaginationPresenterImpl<EntityChanged> implements AdminAuditView.Presenter {
    ...
    public AdminAuditPresenter(KornellSession session, PlaceController placeController, Place defaultPlace,
    ...
    private void getEntitiesChanged() {
        if (StringUtils.isNone(searchTerm)) {
            ...
            session.events().getEntityChangedEvents(searchTerm, pageSize, pageNumber,
                    new Callback<EntityChangedEventsTO>() {
                        @Override
    ...
    public String getClientPropertyName(String property) {
        return session.getAdminHomePropertyPrefix() + "audit" + ClientProperties.SEPARATOR + property;
    }

    @Override
    public int getTotalRowCount() {
        return StringUtils.isNone(getSearchTerm()) ? entityChangedEventsTO.getCount()
                : entityChangedEventsTO.getSearchCount();
    }
